...
from wasserstein import ClassCondSWDLoss
...
# --- Class-conditional SWD (cc_swd) plug-in -----------------------------------
# Extends DEEN's CPM (per-class mean matching) to per-class full-distribution
# matching via sliced Wasserstein. See dcm_reid/src/losses/wasserstein.py.
...
if dataset == 'sysu':
    data_path = '/hy-tmp/datasets/sysumm01/'
    log_path = args.log_path + 'sysu_log/'
    test_mode = [1, 2]  # thermal to visible
    pool_dim = 2048
elif dataset == 'regdb':
    data_path = '/hy-tmp/datasets/'
    log_path = args.log_path + 'regdb_log/'
    test_mode = [2, 1]  # visible to thermal
    pool_dim = 1024
elif dataset == 'llcm':
    data_path = '/hy-tmp/datasets/LLCM/'
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        if batch_idx % 50 == 0:
            ...
            print('Epoch: [{}][{}/{}] '
                  ...
                  'OLoss:{ort_loss.val:.3f}{extra} '.format(
                epoch, batch_idx, len(trainloader),
                train_loss=train_loss, id_loss=id_loss, tri_loss=tri_loss,
                cpm_loss=cpm_loss, ort_loss=ort_loss, extra=extra))
